...
/**
 * Generate PDF for LaborWhtCertificate — same ภ.ง.ด.3 / ภ.ง.ด.53 layout as
 * WhtCertificatePdfService (main app), copy-adapted for the Labor module.
 */
class LaborWhtCertificatePdfService
{
    ...
    public function generate(LaborWhtCertificate $cert): string
    {
        $pdf = new Fpdi();
        $this->setupFont($pdf);
        $pdf->AddPage('P', 'A4');
    ...
    public function generateAndStore(LaborWhtCertificate $cert): string
    {
        $binary = $this->generate($cert);
        $path = sprintf(
            'labor_wht_certificates/%04d/%02d/%s.pdf',
    ...
    // ---------- Layout helpers (mirrors WhtCertificatePdfService) ----------
    ...
    protected function renderHeader(Fpdi $pdf, LaborWhtCertificate $cert): void
    {
        $pdf->SetXY(10, 12);
        ...
        if ($cert->bill) {
            $pdf->Cell(0, 5, $this->txt('อ้างอิงใบวางบิล: ' . $cert->bill->bill_no), 0, 1, 'L');
        }
    ...
    protected function renderParties(Fpdi $pdf, LaborWhtCertificate $cert): void
    {
        $pdf->SetY($pdf->GetY() + 5);
    ...
    protected function renderIncomeTable(Fpdi $pdf, LaborWhtCertificate $cert): void
    {
        $pdf->Ln(4);
        $pdf->SetFont('THSarabunNew', '', 12);
        ...
        $pdf->Cell(35, 9, number_format((float) $cert->amount_paid, 2), 1, 0, 'R');
        $pdf->Cell(20, 9, rtrim(rtrim((string) $cert->wht_rate, '0'), '.'), 1, 0, 'C');
        $pdf->Cell(20, 9, number_format((float) $cert->wht_amount, 2), 1, 1, 'R');

        $pdf->SetFillColor(240, 240, 240);
        $pdf->SetFont('THSarabunNew', '', 13);
        $pdf->Cell(150, 9, $this->txt('รวมภาษีที่หัก ณ ที่จ่าย (Total WHT)'), 1, 0, 'R', true);
        $pdf->Cell(40, 9, number_format((float) $cert->wht_amount, 2), 1, 1, 'R', true);
    }

    protected function renderFooter(Fpdi $pdf, LaborWhtCertificate $cert): void
    {
        $pdf->Ln(8);

        $pdf->SetFont('THSarabunNew', '', 11);
        $signerLabel = 'ผู้มีหน้าที่หักภาษี ณ ที่จ่าย';
    ...
    protected function renderWatermark(Fpdi $pdf, LaborWhtCertificate $cert): void
    {
