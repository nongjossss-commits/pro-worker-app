...
class RenewalController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $steps = RenewalStep::orderBy('order')->get();
        ...
        $employeeQuery = Employee::query()
            ...
            ->with(['renewalSteps' => function($q) {
                $q->select('renewal_steps.id', 'renewal_steps.order', 'employee_renewal_status.employee_id'); // Lightweight Relation
            }]);
        ...
        foreach ($allEmployees as $emp) {
            ...
            if ($emp->status === 'renewal_cancelled') {
                ...
                // Not Started Logic
                if ($stepOneId && !$emp->renewalSteps->contains('id', $stepOneId)) {
                    $notStartedCount++;
                }

                // Step Stats (Highest Step)
                $highestStep = $emp->renewalSteps->sortByDesc('order')->first();
            ...
            $filteredEmployerIds = $filteredEmployerIds->filter(function($empId) use ($employeesByEmployer, $filter, $stepOneId) {
                ...
                if ($filter === 'not_started') {
                    return $emps->contains(function($e) use ($stepOneId) {
                         return $e->status !== 'renewal_cancelled' && !$e->renewalSteps->contains('id', $stepOneId);
                    });
                }
                if ($filter === 'saved') {
                     return $emps->contains('status', 'renewal_completed');
                }
                if ($filter === 'cancelled') {
                     return $emps->contains('status', 'renewal_cancelled');
                }
                // Step Filter
                return $emps->contains(function($e) use ($filter) {
                     if ($e->status === 'renewal_cancelled') return false;
                     $h = $e->renewalSteps->sortByDesc('order')->first();
        ...
        if ($request->input('filter') === 'cancelled_employer') {
            $employers = $employers->filter(function($emp) {
                return $emp->financeOrder->status === 'renewal_resolution_cancelled';
            });
        }
    ...
    public function fetchEmployees(Request $request, Employer $employer)
    {
        ...
        $query = $employer->employees()
            ->whereIn('status', ['renewal_pending', 'renewal_completed', 'renewal_cancelled'])
            ->with(['renewalSteps', 'customFields']); // Load everything needed for the card
        ...
        if ($request->has('filter') && $request->filter) {
            ...
            if ($filter === 'not_started') {
                 $query->where('status', '!=', 'renewal_cancelled')
                       ->whereDoesntHave('renewalSteps', function($q) use ($stepOneId) {
                           $q->where('renewal_steps.id', $stepOneId);
                       });
            } elseif ($filter === 'saved') {
        ...
        if ($request->has('filter') && is_numeric($request->filter)) {
            ...
            $employees = $employees->filter(function($emp) use ($filterStepId) {
                ...
                $highest = $emp->renewalSteps->sortByDesc('order')->first();
    ...
    public function cancelEmployer(Request $request, Employer $employer)
    {
        ...
        DB::transaction(function () use ($employer) {
            ...
            if ($order) {
                $order->update(['status' => 'renewal_resolution_cancelled']);
            }
    ...
    public function restoreEmployer(Request $request, Employer $employer)
    {
        ...
        DB::transaction(function () use ($employer) {
            ...
            if ($order) {
                $order->update(['status' => 'renewal_resolution']);
            }
    ...
    public function finalize(Request $request, Employee $employee)
    {
        ...
        // Change status to 'renewal_completed'
    ...
    public function cancel(Request $request, Employee $employee)
    {
        ...
        return back()->with('success', 'Employee renewal cancelled.');
    }
    ...
    public function bulkFinalize(Request $request)
    {
        ...
        Employee::whereIn('id', $request->input('employee_ids'))
            ->update(['status' => 'renewal_completed']);
    ...
    /**
     * Import employees by expiry date.
     */
    public function importByExpiry(Request $request)
    {
        if (!auth()->user()->can('edit-employees')) {
            abort(403);
        }
        ...
        $count = 0;
        DB::transaction(function() use ($targetDate, &$count) {
            // Find employees matching criteria
            $employees = Employee::where(function($q) use ($targetDate) {
                    $q->whereDate('workPermitExpiryDate', $targetDate)
                      ->orWhereDate('visaExpiryDate', $targetDate);
                })
                ->where(function($q) {
                    $q->whereNull('workPermitMOUGroup')
                      ->orWhere('workPermitMOUGroup', '!=', 'MOU');
                })
                // Ensure we don't mess up employees already in a resolution/renewal process if active?
                // Or maybe we WANT to move them. Let's assume we move anyone matching.
                // But maybe filter out deleted ones.
                ->get();
        ...
        if ($request->ajax()) {
            return response()->json([
                'success' => true,
                'message' => "Imported {$count} employees for renewal.",
                'count' => $count
            ]);
        }
    ...
    public function store(Request $request)
    {
        ...
        // Reusing same validation as Registration but mapped for Renewal context if needed.
        // For simplicity, we use same validation rules.
        // Copy of validation from RegistrationController@store
         $validated = $request->validate([
    ...
    public function storeStep(Request $request)
    {
        ...
        $maxOrder = RenewalStep::max('order') ?? 0;
     ...
     */
    public function updateStep(Request $request, RenewalStep $step)
    {
        $validated = $request->validate([
            'name' => 'required|string|max:255',
        ]);
     ...
     * Delete a step.
     */
    public function destroyStep(RenewalStep $step)
    {
        $step->delete();
        return response()->json(['success' => true]);
    }
    ...
    public function updateProgress(Request $request, Employee $employee)
    {
        ...
        try {
            $validated = $request->validate([
                'step_id' => 'required|exists:renewal_steps,id',
            ...
            if ($validated['completed']) {
                $employee->renewalSteps()->syncWithoutDetaching([
                    $validated['step_id'] => ['completed_at' => now()]
                ]);
            } else {
                $employee->renewalSteps()->detach($validated['step_id']);
            }
    ...
    /**
     * Update employer renewal resolution status and note.
     */
    public function updateResolutionStatus(Request $request, Employer $employer)
    {
        ...
        if ($request->has('status')) {
            $data['renewal_resolution_status'] = $validated['status'];
        }
        if ($request->has('note')) {
            $data['renewal_resolution_note'] = $validated['note'];
        }
    ...
    /**
     * Update employer renewal custom field.
     */
    ...
    private function getStats($employerId = null, Request $request = null)
    {
        ...
        $allStatuses = ['renewal_pending', 'renewal_completed', 'renewal_cancelled'];
        ...
        // 3. Cancelled
        $globalCancelled = (clone $globalQuery)->where('status', 'renewal_cancelled')->count();

        // 4. Saved (Completed)
        $globalSaved = (clone $globalQuery)->where('status', 'renewal_completed')->count();

        // 5. Total Employers (who have any renewal employees)
        ...
        if ($employerId) {
            ...
            $empCancelled = (clone $empQuery)->where('status', 'renewal_cancelled')->count();
            $empSaved = (clone $empQuery)->where('status', 'renewal_completed')->count();
