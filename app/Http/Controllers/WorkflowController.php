...
class WorkflowController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('search') && $request->search) {
            ...
            if (preg_match('/^ID:\s*(\d+)$/i', $search, $matches)) {
                ...
                $query->where(function($q) use ($targetId) {
                    $q->whereHas('items', function($itemQuery) use ($targetId) {
                        $itemQuery->whereHas('employee', function($emp) use ($targetId) {
                            $emp->where('id', $targetId);
                        });
                    })
                    ->orWhereHas('employer', function($e) use ($targetId) {
                        $e->where('id', $targetId);
                    });
                });
            } else {
        ...
        if ($request->has('filter') && $request->filter) {
            ...
            $query->whereHas('items', function($q) use ($filter) {
                if ($filter === 'not_started') {
                    ...
                    // whereHas would also match items that passed this step
                    // and moved on to a later one, which is why employers
                    // with a "step X = 0" pill were still leaking in.
        ...
        if ($activeTab) {
            ...
            if ($matchingOrderIds->isNotEmpty()) {
                ...
                // completed items keep counting until 24h after Finish (same
                // grace window the item card enjoys in the visible list).
                // Was previously `whereNotIn(['cancelled','completed'])`,
                // which decremented the badge the instant Finish was pressed
                // even though the card itself was still on screen — the two
                // views disagreed for 24 hours.
    ...
    private function calculateTabStats($workType, $isPreProduction)
    {
        ...
        foreach ($allOrders as $order) {
            ...
            foreach ($order->items as $item) {
                ...
                // 24h after Finish (same window the card enjoys in the list).
    ...
    private function calculateOrderStats(ProductionOrder $order)
    {
        ...
        foreach ($items as $item) {
            ...
            // Step badge — 24h grace: completed items keep counting until 24h
            // after Finish (same window the card enjoys in the visible list).
            $completedAt = $item->completed_at;
            $countTowardsSteps = !($item->status === 'completed'
                && $completedAt
                && $completedAt->lt($graceCutoff));
     ...
     * "Select All" can pick up every eligible employee even if the card was
     * never expanded, without the heavy financial-status computation or
     * HTML rendering that fetchEmployees() does for the full card view.
     */
    ...
    public function batchStats(Request $request)
    {
        ...
        foreach ($orders as $order) {
            ...
            foreach ($items as $item) {
                ...
                // Per-order step badge — 24h grace matches the global stats:
                // completed items whose 24h countdown finished no longer
                // count toward the per-order badge either.
                $completedAt = $item->completed_at;
                $countTowardsSteps = !($item->status === 'completed'
                    && $completedAt
                    && $completedAt->lt($graceCutoff));
