                ...
                <div class="card mb-4 sticky-top" style="top: 20px;">
                    ...
                    <div class="card-body">
                        ...
                        <div x-show="isUploading" class="mb-3">
                            <div class="progress" style="height: 25px;">
                                <div class="progress-bar progress-bar-striped progress-bar-animated bg-secondary" role="progressbar" :style="'width: ' + uploadProgress + '%'">
                                    {{ __('กำลังอัปโหลด (') }}<span x-text="filesUploadedCount"></span> / <span x-text="filesToUploadCount"></span>)...
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-grid gap-2 mb-3" :class="{ 'opacity-50': isUploading || !contextEmployerId }">
                            <button type="button" class="btn btn-outline-primary" @click="openExistingEmployeeModal" :disabled="isUploading || !contextEmployerId" title="{{ __('กรุณาเลือกนายจ้างก่อน') }}">
                                <i class="bi bi-person-check me-2"></i> {{ __('แนบลูกจ้างที่มีอยู่') }}
                            </button>

                            {{-- V2.5-S17: New Button for External Employees --}}
                            <button type="button" class="btn btn-outline-warning text-dark" @click="openGlobalEmployeeSearch" :disabled="isUploading" title="{{ __('ค้นหาลูกจ้างทั้งระบบ') }}">
                                <i class="bi bi-search me-2"></i> {{ __('แนบลูกจ้างภายนอก') }}
                            </button>

                            <button type="button" class="btn btn-outline-success" @click="openNewEmployeeModal" :disabled="isUploading || !contextEmployerId" title="{{ __('กรุณาเลือกนายจ้างก่อน') }}">
                                <i class="bi bi-person-plus me-2"></i> {{ __('แจ้งเข้าลูกจ้างใหม่') }}
                            </button>
                            <div class="d-flex gap-2">
                                <button type="button" class="btn btn-outline-secondary flex-grow-1" @click="triggerScanner" :disabled="isUploading" title="{{ __('สแกนเอกสาร') }}">
                                    <i class="bi bi-camera me-2"></i> {{ __('สแกน') }}
                                </button>
                                <button type="button" class="btn btn-outline-secondary flex-grow-1" @click="triggerFileInput" :disabled="isUploading" title="{{ __('แนบไฟล์ทั่วไป') }}">
                                    <i class="bi bi-file-earmark-arrow-up me-2"></i> {{ __('ไฟล์') }}
                                </button>
                            </div>
                        </div>
                        <hr>
