...
<body>
    <div class="no-print action-bar">
        ...
        <div>
            <button class="btn" onclick="window.print()">{{ __('Print / Save PDF') }}</button>
            <button class="btn" onclick="window.close()" style="margin-left: 10px;">{{ __('Close') }}</button>
        </div>
    </div>
    ...
    <div class="page">
        ...
        <table class="header-table">
            <tr>
                <td style="width: 55%;">
                    ...
                    @if(isset($billerProfile) ? $billerProfile->phone : $profile->{{ __('phone)') }}<div class="tax-id">Tel: {{ isset($billerProfile) ? $billerProfile->phone : $profile->phone }}</div>@endif
                </td>
                <td style="width: 45%;" class="doc-title">
                    ...
                    <table class="meta-table">
                        ...
                        <tr>
                            <td class="meta-label">{{ __('Date') }}<span class="en-label">{{ __('/ วันที่') }}</span>:</td>
                            <td>{{ date('d/m/Y') }}</td>
                        </tr>
                        <tr>
        ...
        <div class="client-box">
            <div class="client-label">{{ __('Bill To') }}<span class="en-label">{{ __('/ ลูกค้า') }}</span></div>
        ...
        <table class="items-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 60%;">{{ __('Description') }}<span class="en-label">{{ __('/ รายการ') }}</span></th>
                    <th style="width: 10%;" class="text-center">{{ __('Qty') }}<span class="en-label">{{ __('/ จำนวน') }}</span></th>
                    <th style="width: 10%;" class="text-right">{{ __('Unit Price') }}<span class="en-label">{{ __('/ ราคา') }}</span></th>
                    <th style="width: 15%;" class="text-right">{{ __('Amount') }}<span class="en-label">{{ __('/ รวม') }}</span></th>
                </tr>
            </thead>
            <tbody>
                <!-- SERVICE FEE SECTION -->
                ...
                @if($showAdvance)
                    @if($advanceTransactions->{{ __('isNotEmpty())') }}<tr>
                    ...
                    @elseif(!$hasSpecificTransactions && isset($advanceItems) && $advanceItems->{{ __('isNotEmpty())') }}<tr>
                            <td colspan="5" class="section-header" style="background-color: #fff7ed; color: #ea580c;">{{ __('Advance Payments (เงินสำรองจ่าย)') }}<span style="font-size: 10px; font-weight: normal; color: #666;">{{ __('(No VAT)') }}</span></td>
                        </tr>
        ...
        <div class="totals-container">
            <table class="totals-table">
                @if($showService)
                    <tr>
                        <td class="total-label"><strong>{{ __('Service Base') }}<span class="en-label">{{ __('/ มูลค่าบริการ (ก่อน VAT)') }}</span></strong></td>
                    ...
                    @endif
                    <tr>
                        <td class="total-label" style="border-bottom: 1px solid #ddd;">{{ __('Service Total') }}<span class="en-label">{{ __('/ รวมค่าบริการ (รวม VAT)') }}</span></td>
                ...
                @if($showAdvance && $advanceTotal > 0)
                    <tr>
                        <td class="total-label" style="color: #ea580c;"><strong>{{ __('Total Advance Payments') }}<span class="en-label">{{ __('/ รวมเงินสำรองจ่าย') }}</span></strong></td>
                ...
                <tr class="grand-total-row">
                    <td>{{ __('Grand Total') }}<span class="en-label">{{ __('/ รวมทั้งสิ้น') }}</span></td>
                ...
                <tr style="font-weight: bold; border-top: 1px dashed #ccc;">
                    <td class="total-label" style="padding-top: 5px;">{{ __('Net Payable') }}<span class="en-label">{{ __('/ ยอดสุทธิ') }}</span></td>
        ...
        <div class="signatures-container">
            ...
            <div class="signatures">
                <div class="sig-block">
                    <div class="sig-title">{{ __('Received By') }}<span class="en-label">{{ __('/ ผู้รับเงิน') }}</span></div>
                    <div class="sig-line"></div>
                    <div class="sig-text">{{ __('Date') }}<span class="en-label">{{ __('/ วันที่') }}</span>: ____/____/______</div>
                </div>

                <div class="sig-block">
                    <div class="sig-title">{{ __('Authorized Signature') }}<span class="en-label">{{ __('/ ผู้มีอำนาจลงนาม') }}</span></div>
            ...
            <!-- Footer -->
            <div class="footer">{{ __('Thank you for your business.') }}<br>{{ __('Please check the correctness of this document.') }}</div>
        </div>
    </div>
    ...
    <div class="page" style="page-break-before: always; margin-top: 20px;">
        ...
        <table class="items-table" style="font-size: 12px;">
            <thead>
                <tr>
                    <th style="width: 5%; text-align: center;">{{ __('ลำดับ') }}<br><span class="en-label">{{ __('No.') }}</span></th>
                    <th style="width: 15%; text-align: center;">{{ __('รูปถ่าย') }}<br><span class="en-label">{{ __('Photo') }}</span></th>
                    <th style="width: 15%;">{{ __('รหัสลูกจ้าง') }}<br><span class="en-label">{{ __('Employee ID') }}</span></th>
                    <th style="width: 35%;">{{ __('ชื่อ-นามสกุล') }}<br><span class="en-label">{{ __('Name') }}</span></th>
                    <th style="width: 15%; text-align: center;">{{ __('สัญชาติ') }}<br><span class="en-label">{{ __('Nationality') }}</span></th>
                    <th style="width: 15%; text-align: right;">{{ __('ราคาต่อหัว') }}<br><span class="en-label">{{ __('Price') }}</span></th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align: center; vertical-align: middle;">
                        ...
                        @else
                            <div style="width: 50px; height: 50px; background-color: #f3f4f6; border-radius: 4px; display: inline-block; line-height: 50px; color: #9ca3af; font-size: 10px;">{{ __('No Image') }}</div>
                        @endif
                    </td>
