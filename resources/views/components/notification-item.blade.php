...
@php
    ...
    if (!function_exists('getNotificationPresentationInfo')) {
        function getNotificationPresentationInfo($dueDate) {
            if (!$dueDate) {
                return (object)['daysRemainingText' => __('ไม่มีข้อมูล'), 'alertClass' => 'alert-secondary'];
            }
    ...
    <div class="d-flex align-items-center gap-3">
        ...
        <div class="d-flex justify-content-between align-items-start w-100">
            <div class="flex-grow-1">
                ...
                @if(isset($employee->employer))<p class="mb-1"><strong>{{ __('นายจ้าง:') }}</strong> {{ $employee->employer->employerNameTh ?? 'N/A' }}</p>@endif
            ...
            <div class="text-end flex-shrink-0 ms-2">
                ...
                <div class="btn-group btn-group-sm">
                    <button type="button" class="btn btn-info" title="{{ __('ดูข้อมูล') }}"><i class="bi bi-search"></i></button>
                    <button type="button" class="btn btn-success" title="{{ __('ต่ออายุ') }}"><i class="bi bi-calendar-check"></i></button>
