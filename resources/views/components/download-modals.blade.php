                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check">
                            ...
                            <label class="form-check-label" for="dlTypeZip">
                                {{ __('Separate Files (ZIP)') }}
                                <small class="d-block text-muted">{{ __('Creates folders for each employee with individual files.') }}</small>
                            </label>
                        </div>
                        <div class="form-check mt-2">
                            ...
                            <label class="form-check-label" for="dlTypePdf">
                                {{ __('Merged PDF') }}
                                <small class="d-block text-muted">{{ __('Combines all selected files into a single PDF per employee (or one giant PDF).') }}</small>
                            </label>
                        </div>
                    </div>
...
<div class="modal fade" id="downloadCenterModal" tabindex="-1" aria-labelledby="downloadCenterModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-muted">{{ __('Background tasks are listed here. Click \'Download\' when ready.') }}</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btnConfirmDownload').addEventListener('click', function() {
        ...
        if (files.length === 0) {
            showToast('{{ __('Please select at least one file type') }}', 'danger');
            return;
        }

        const btn = this;
        btn.disabled = true;
        btn.innerText = '{{ __('Starting...') }}';
        ...
        .then(data => {
            if (data.success) {
                ...
                showToast('{{ __('Error starting download') }}', 'danger');
            }
        })
        .catch(err => {
            console.error(err);
            showToast('{{ __('An unexpected error occurred') }}', 'danger');
        })
        .finally(() => {
            btn.disabled = false;
            btn.innerText = '{{ __('Start Download') }}';
        });
    });
    ...
    function loadDownloadTasks() {
        ...
        if (!tbody.innerHTML.trim() || tbody.innerText.includes('{{ __('Loading...') }}') || tbody.innerText.includes('{{ __('No tasks found') }}')) {
        ...
        fetch('{{ route("admin.downloads.index") }}')
            ...
            .then(tasks => {
                ...
                tasks.forEach(task => {
                    ...
                    if (task.status === 'completed') {
                        ...
                        actionBtn = `<a href="${url}" class="btn btn-sm btn-success" target="_blank"><i class="bi bi-download"></i> {{ __('Download') }}</a>`;
            ...
            .catch(err => {
                ...
                if(tbody.children.length === 1 && tbody.innerText.includes('{{ __('Loading...') }}')) {
