...
<div class="container-fluid py-4" x-data="preparationManager()">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <div class="text-uppercase text-muted small fw-bold">{{ __('Pre-Production Stage') }}</div>
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('production.index') }}" class="btn btn-outline-secondary">{{ __('Exit') }}</a>
            <form action="{{ route('production.update', $production->id) }}" method="POST" id="startWorkflowForm">
                ...
                <button type="submit" class="btn btn-success btn-lg shadow-sm"
                    ...
                    <i class="bi bi-send-check me-2"></i>{{ __('Send to Workflow') }}</button>
            </form>
        </div>
    </div>
    ...
    <div class="row mb-4">
        <div class="col-md-6">
            <div class="card shadow-sm border-0 h-100" :class="{'bg-success text-white': documentReady, 'bg-light': !documentReady}">
                <div class="card-body d-flex align-items-center justify-content-between">
                    <div>
                        <h5 class="fw-bold mb-1"><i class="bi bi-file-earmark-check me-2"></i>{{ __('Documents Ready') }}</h5>
                        <div class="small" :class="{'text-white-50': documentReady, 'text-muted': !documentReady}">{{ __('Checked by Staff / Assignee') }}</div>
                    </div>
        ...
        <div class="col-md-6">
            ...
            <div class="card shadow-sm border-0 h-100" :class="{'bg-success text-white': financialApproved, 'bg-light': !financialApproved}">
                <div class="card-body d-flex align-items-center justify-content-between">
                    <div>
                        <h5 class="fw-bold mb-1"><i class="bi bi-cash-coin me-2"></i>{{ __('Ready to Proceed') }}</h5>
                        <div class="small" :class="{'text-white-50': financialApproved, 'text-muted': !financialApproved}">{{ __('Financial / Admin Approval') }}</div>
                    </div>
                    <div class="form-check form-switch">
                        <input class="form-check-input fs-4" type="checkbox" role="switch"
    ...
    <ul class="nav nav-tabs mb-4" id="productionTabs" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="details-tab" data-bs-toggle="tab" data-bs-target="#details" type="button" role="tab" aria-selected="true">{{ __('Project Details & Employees') }}</button>
        </li>
        @can('view-finance')
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="financial-tab" data-bs-toggle="tab" data-bs-target="#financial" type="button" role="tab" aria-selected="false">{{ __('Financial Management') }}</button>
        </li>
        @endcan
    </ul>
            ...
            <div class="row">
                ...
                <div class="col-lg-4 mb-4">
                    <div class="card shadow-sm border-0 mb-4">
                        <div class="card-header bg-white fw-bold">{{ __('Project Details') }}</div>
                        <div class="card-body">
                            <form action="{{ route('production.update', $production->id) }}" method="POST">
                                ...
                                <button type="submit" class="btn btn-sm btn-primary w-100 mt-2">{{ __('Save Details') }}</button>
                            </form>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
                            ...
                            <div class="dropdown">
                                <button class="btn btn-primary btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown">
                                    <i class="bi bi-person-plus me-1"></i>{{ __('Add Employee') }}</button>
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="{{ route('employees.import', ['production_id' => $production->id]) }}"><i class="bi bi-file-earmark-spreadsheet me-2"></i>{{ __('Import from Excel') }}</a></li>
                                    <li><hr class="dropdown-divider"></li>
                                    <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#addExistingModal">{{ __('Select Existing (DB)') }}</a></li>
                                    <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#addNewModal">{{ __('Create New (Manual)') }}</a></li>
                                </ul>
                            </div>
                        </div>
                        <div class="card-body bg-light">
                            @if($production->items->{{ __('isEmpty())') }}<div class="text-center py-5 text-muted">
                                    <i class="bi bi-people display-4"></i>
                                    <p class="mt-2">{{ __('No employees added to this project yet.') }}</p>
                                </div>
                            @else
                                @php
                                ...
                                @if($newEmployees->{{ __('isNotEmpty())') }}<h6 class="text-primary fw-bold mb-3"><i class="bi bi-person-plus-fill me-2"></i>{{ __('New / Imported Employees') }}</h6>
                                                ...
                                                <div class="card h-100 border shadow-sm employee-card-production">
                                                    <div class="card-body d-flex align-items-start">
                                                        ...
                                                        <div class="flex-grow-1 overflow-hidden">
                                                            ...
                                                            <div class="small text-muted mb-1 text-truncate">{{ __('New Import') }}</div>
                                                            ...
                                                            <div>
                                                                <span class="badge bg-info text-dark"><i class="bi bi-star-fill me-1"></i>{{ __('New Entry') }}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                @if($existingEmployees->{{ __('isNotEmpty())') }}<h6 class="text-secondary fw-bold mb-3"><i class="bi bi-database-check me-2"></i>{{ __('Existing Database Employees') }}</h6>
                                                        ...
                                                        <div class="flex-grow-1 overflow-hidden">
                                                            ...
                                                            <div>
                                                                @if($emp->status === 'pending_confirmation')
                                                                    <span class="badge bg-warning text-dark"><i class="bi bi-hourglass-split me-1"></i>{{ __('Pending Workflow') }}</span>
                                                                @else
                                                                    <span class="badge bg-secondary">{{ __('Pending Workflow') }}</span>
                                                                @endif
                                                            </div>
                                                        </div>
                                                    </div>
        ...
        <form action="{{ route('production.add_employee', $production->id) }}" method="POST">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('Add Existing Employee') }}</h5>
...
<div class="modal fade" id="addNewModal" tabindex="-1">
    <div class="modal-dialog">
        <form action="{{ route('production.add_new_employee', $production->id) }}" method="POST">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('Create New Employee (External/Import)') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                    ...
                    <div class="row g-2 mb-3">
                        ...
                        <div class="col-6">
                            <label class="form-label">{{ __('Surname (TH)') }}</label>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                    <button type="submit" class="btn btn-primary">{{ __('Create & Add') }}</button>
                </div>
            </div>
        </form>
    </div>
