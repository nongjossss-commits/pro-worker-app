...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <div class="text-uppercase text-primary small fw-bold">{{ __('Active Workflow Tracking') }}</div>
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('production.index') }}" class="btn btn-outline-secondary">
                <i class="bi bi-arrow-left me-1"></i>{{ __('Back to Dashboard') }}</a>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-3">
            <div class="card shadow-sm border-0 bg-primary text-white h-100">
                <div class="card-body">
                    <div class="small opacity-75">{{ __('Total Employees') }}</div>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-header bg-white py-3">
            <h5 class="mb-0 fw-bold">{{ __('Employee Tracking List') }}</h5>
        </div>
                            ...
                            <tr>
                                ...
                                <td class="text-end pe-4">
                                    <a href="{{ route('workflow.item.show', $item->id) }}" class="btn btn-primary btn-sm">
                                        <i class="bi bi-eye-fill me-1"></i>{{ __('Open Timeline') }}</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-5">{{ __('No employees in this project.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
