...
<div class="container-fluid">
    ...
    <div class="card shadow-sm border-0 mb-4 bg-white">
        <div class="card-body p-3">
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-center gap-3">
                <h4 class="mb-0 text-primary fw-bold text-nowrap"><i class="bi bi-people-fill me-2"></i>{{ __('Renewal Resolution') }}</h4>
    ...
    <div class="accordion" id="employersAccordion">
        @foreach($employers as $employer)
            @php
                $isEmployerCancelled = $employer->financeOrder && $employer->financeOrder->status === 'renewal_resolution_cancelled';
            ...
            <div class="d-flex align-items-start employer-card-container mb-4" id="employer-card-{{ $employer->id }}">
                ...
                <div class="card flex-grow-1 shadow-sm overflow-visible {{ $employerCardClass }}" style="position: relative;">
                    ...
                    <div class="position-absolute d-flex align-items-center gap-1 shadow-sm border border-secondary border-bottom-0 rounded-top bg-white px-2 py-1"
                        ...
                        {{-- Status Dropdown --}}
                        @php
                            $status = $employer->renewal_resolution_status ?? 'preparing';
                        ...
                        <button class="btn btn-sm btn-outline-secondary border-0"
                                data-note="{{ $employer->renewal_resolution_note ?? '' }}"
...
{{-- Import By Expiry Modal (NEW) --}}
<div class="modal fade" id="importByExpiryModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('production.renewal.import.expiry') }}" method="POST">
                @csrf
                <div class="modal-body p-4">
                    ...
                    <div class="mb-3">
                        <label for="target_date" class="form-label fw-bold">{{ __('Target Expiry Date') }}</label>
                        <input type="date" class="form-control form-control-lg" id="target_date" name="target_date" required>
                    </div>
                </div>
...
<script>
    ...
    window.updateResolutionStatus = function(employerId, status) {
        fetch(`/production/renewal/employer/${employerId}/resolution-status`, {
            method: 'POST',
            headers: {
    ...
    document.addEventListener('change', function(e) {
        if (e.target.classList.contains('employer-select-all')) {
            ...
            checkboxes.forEach(cb => {
                ...
                const isPending = (status === 'renewal_pending');
             ...
             if (result.isConfirmed) {
                ...
                .then(data => {
                    if(data.success) {
                        ...
                        if(card) {
                            card.dataset.status = 'renewal_completed';
             ...
             if (result.isConfirmed) {
                ...
                .then(data => {
                    if(data.success) {
                        const card = document.getElementById(`employee-card-${id}`);
                        if(card) {
                            card.dataset.status = 'renewal_pending';
             ...
             if (result.isConfirmed) {
                ...
                .then(data => {
                    if(data.success) {
                        const card = document.getElementById(`employee-card-${id}`);
                        if(card) {
                            card.dataset.status = 'renewal_cancelled';
                            card.dataset.isNotStarted = 'false';
                            ...
                            Swal.fire('{{ __('Cancelled') }}', '{{ __('Renewal cancelled.') }}', 'success');
