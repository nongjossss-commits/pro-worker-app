...
@section('content')
<x-help-button manual="registration_resolution" title="{{ __('Registration Resolution') }}" />
<style>
...
<div class="container-fluid">
    ...
    {{-- Renewal-Progress Filter Pills — multi-select toggle.
         Each pill click adds or removes its state from `renewal_filters`
         (comma-separated). The same pill clicked twice deselects it; the
         Clear button strips the whole filter group.
    --}}
    @php
        ...
        // Strip the legacy single-select `filter=renewal_*` value if it's
        // lingering in the URL. The renewal pills now use `renewal_filters`
        // (multi-select) and the single-select form has no handler anymore.
...
<x-manage-steps-modal
    ...
    :store-url="route('production.registration.steps.store', ['resolutionTab' => $currentTab->id])"
    :update-url-base="url('production/registration/' . $currentTab->id . '/steps')"
    :destroy-url-base="url('production/registration/' . $currentTab->id . '/steps')"
    :reorder-url="route('production.registration.steps.reorder', ['resolutionTab' => $currentTab->id])"
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="autoMouInput">
                        ...
                        @foreach(\App\Models\WorkPermitType::ordered()->get() as $wpType)
                            <option value="{{ $wpType->name }}" {{ ($resolutionSettings['registration_auto_mou_group'] ?? '') == $wpType->name ? 'selected' : '' }}>{{ $wpType->name }}</option>
                        @endforeach
...
<script>
    ...
    document.addEventListener('change', function(e) {
        if (e.target.classList.contains('employer-select-all')) {
            ...
            // (that one keeps its existing DOM-based behaviour, unchanged).
            if (!e.isTrusted) return;
            const employerId = e.target.dataset.employerId;
            const endpoint = `/production/registration/{{ $currentTab->id }}/employer/${employerId}/select-all-ids`;
